...
class Lanedecttest(Node):
    def __init__(self):
        ...
        # clipping distance: 1미터를 센서 단위로 변환
        clipping_distance_in_meters = 1.0  # 미터
    ...
    def image_processing(self):
        ...
        if max_contour is not None:
            ...
            # 깊이 마스킹 (0.3m ~ clipping distance 미터)
            min_depth_raw = int(0.3 / self.depth_scale)  # 0.3m → 센서 raw 단위 환산
            max_depth_raw = int(self.clipping_distance)  # clipping_distance는 raw 단위 (1m)
            depth_mask = np.where((roi_depth > min_depth_raw) & (roi_depth < max_depth_raw), 255, 0).astype(np.uint8)
