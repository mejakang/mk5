...
class Lanedecttest(Node):
    ...
    def hsv_detection(self, img):
        ...
        # 파란색 HSV 범위 (조정 가능)
        lower_blue = np.array([90, 100, 100])# 90 50 50
        ...
        if max_contour is not None:
            ...
            filtered = cv2.bitwise_and(img, img, mask=contour_mask)
            cv2.imshow("Filtered Blue", filtered)
            cv2.waitKey(1)
    ...
    def image_processing(self):
        ...
        max_contour, filtered = self.hsv_detection(roi)
        ...
        cv2.imshow("ROI", roi)
        cv2.imshow("Filtered Blue + Depth Masked", filtered)
        cv2.waitKey(1)
