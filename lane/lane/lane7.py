...
class Lane(Node):
    def __init__(self):
        ...
        #color_sensor.set_option(rs.option.white_balance, 5800)  # 수동 화이트밸런스 예시
    ...
    def image_capture(self):
        ...
        # 필터 적용 순서: temporal -> spatial -> hole filling
        depth_filtered = self.temporal_filter.process(depth_frame)
        ...
        self.depth_intrinsics = self.filled_depth_frame.profile.as_video_stream_profile().intrinsics
    ...
    def yuv_detection(self, img):
        ...
        if max_contour is not None:
            ...
            cv2.line(img, (left, 0), (left, img.shape[0]), (0, 255, 255), 1)
            cv2.line(img, (right, 0), (right, img.shape[0]), (0, 255, 255), 1)
        ...
        R_sum = int(np.sum(R_histo) / 255)  # 수정: - y 제거
    ...
    def image_processing(self):
        ...
        cv2.line(self.color_img, (self.center_x, y1), (self.center_x, y2), (255, 0, 0), 2) #이거가 중심선
